...
const ExceptionLog = ({
  ...
  const displayjobname = (data: any) => {
    ...
    return (
      <div>
        <div className="text-[0.833vw] leading-[1.85vh] font-bold dark:text-white">
          {jobName}
        </div>
  ...
  const RenderTableCell = (item: any, column: any) => {
    switch (column?.id) {
      ...
      case "timeStamp":
        return (
          <div className="text-[0.83vw] text-black/50 dark:text-[#FFFFFF]/50 ">
            {item.timeStamp}
          </div>
        );
      case "version":
        return (
          <div className="text-[0.83vw] font-medium dark:text-white">
            {item.version}
          </div>
        );
      case "errorCode":
        return (
          <div className="text-[0.83vw] dark:text-white/50">
            {item.errorCode}
          </div>
        );
      // case "errorDetails":
  ...
  return (
    <div className="flex w-full h-full gap-5">
          ...
          {({
            ...
            <>
                  ...
                  <>
                    {columns.map((column: any, i: number) => (
                      <TorusColumn
                        ...
                        className={`text-[0.72vw] leading-[2.22vh] font-medium bg-[#F4F5FA] cursor-pointer dark:bg-[#0F0F0F] dark:text-[#FFFFFF] ${
              ...
              <TableBody
                ...
                renderEmptyState={() => (
                  <div className="text-center overflow-y-auto dark:bg-[#161616] dark:text-[#FFFFFF]">
                    {" "}
                    No Data{" "}
                  </div>
                )}
                  ...
                  <TorusRow
                    ...
                    className={`${
                      item.timeStamp == tabdata.timeStamp
                        ? "bg-[#F4F5FA] dark:bg-[#0F0F0F] rounded-full"
                        : ""
                    }hover:bg-[#F4F5FA] dark:hover:bg-[#0F0F0F] dark:hover:text-white outline-none hover:cursor-pointer`}
      ...
      <div className="w-[28%] h-[88%] mt-[0.58vw] mr-[0.87vw] items-center bg-[#F4F5FA] rounded-lg dark:bg-[#0F0F0F] dark:text-[#FFFFFF]">
        <p className="text-[0.93vw] leading-[2.22vh] p-[0.87vw] font-semibold text-left">
          Error Details
        </p>
